...
def _gate_host_response(
    ...
    except TrustGateBypassError:
        logger.error("TRUST_GATE_BYPASS_ATTEMPT: boundary assertion triggered")
        raise

    gated_response = gate.gate_response(
        draft_answer_text=adapter.get_draft_answer(host_context),
        retrieved_evidence=adapter.get_retrieved_evidence(host_context),
        context=context,
    )
    payload = adapter.set_final_response(host_context, gated_response)
    assert_no_raw_output(payload)
    return payload


def _build_contract_error_response(
    *,
    request: Request,
    chat_session_id: str | None,
    origin: MessageOrigin | str | None,
    failure_mode: str,
    error_message: str,
) -> dict:
    host_context = {
        ...
        "chat_message_req": SimpleNamespace(stream=False, origin=origin),
    }
    adapter = OnyxHostAdapter()
    metadata = adapter.get_request_metadata(host_context)
    metadata["failure_modes"] = [failure_mode]
    metadata["request_path"] = request.url.path

    return _gate_host_response(host_context, context_override=metadata)
...
def handle_new_chat_message(
    ...
    """
    Deprecated endpoint for sending a chat message.

    Returns:
        dict: Enforced Trust & Evidence response contract.
    """
    tenant_id = get_current_tenant_id()
    ...
    try:
        with get_session_with_current_tenant() as db_session:
            ...
            host_context = {
                "chat_result": SimpleNamespace(
                    ...
                    top_documents=basic_result.top_documents,
                    tool_calls=[],
                    message_id=basic_result.message_id,
        ...
        return _build_contract_error_response(
            ...
            if chat_message_req.chat_session_id
            else None,
...
def handle_send_chat_message(
    ...
    """
    This endpoint is used to send a new chat message and enforce Trust & Evidence output.
